...
export interface ModelCapabilityContext {
  ...
  getPreviewGeminiFlashModel?(): string;
  getDefaultGeminiFlashModel?(): string;
}
...
export function getPreviewGeminiFlashModel(
  config?: ModelCapabilityContext,
): string {
  if (config?.getPreviewGeminiFlashModel) {
    return config.getPreviewGeminiFlashModel();
  }
...
export function getDefaultGeminiFlashModel(
  ...
  if (config?.getDefaultGeminiFlashModel) {
    return config.getDefaultGeminiFlashModel();
  }
...
export function resolveModel(
  ...
  switch (normalizedModel) {
    ...
    case GEMINI_MODEL_ALIAS_FLASH: {
      resolved = useGemini3_5Flash
        ? getDefaultGeminiFlashModel(config)
        : getPreviewGeminiFlashModel(config);
      break;
    }
  ...
  if (
    ...
    normalizedModel !== getPreviewGeminiFlashModel(config)
  ) {
    return getDefaultGeminiFlashModel(config);
  }

  if (!hasAccessToPreview && isPreviewModel(resolved)) {
    // Downgrade to stable models if user lacks preview access.
    switch (resolved) {
      case getPreviewGeminiFlashModel(config):
        return getDefaultGeminiFlashModel(config);
      case PREVIEW_GEMINI_MODEL:
...
export function getDisplayString(
  ...
  switch (model) {
    ...
    case GEMINI_MODEL_ALIAS_FLASH:
      return getPreviewGeminiFlashModel(config);
    case PREVIEW_GEMINI_3_1_CUSTOM_TOOLS_MODEL:
